...
def ler_usuarios(): #Lê todos os usuários do arquivo de login.
    ...
    if not os.path.exists(ARQUIVO_LOGIN): #Cria o arquivo com um usuário padrão se não existir
        with open(ARQUIVO_LOGIN, "w", encoding="utf-8") as f: #a variável f é o objeto arquivo
            writer = csv.writer(f)
            writer.writerow(["administrador@escola.com", "123", "administrador"])
            writer.writerow(["professor@escola.com", "123", "professor"])
            writer.writerow(["aluno@escola.com", "123", "aluno"])
...
def janela_excluir_usuario():
    ...
    ttk.Button(frame, text="Sair", width=25, command=janela.destroy).pack(pady=10)
...
def janela_aluno(login_window, usuario_logado):
    ...
    email = usuario_logado['email']
    tipo = usuario_logado['tipo']
    nome_usuario = email.split("@")[0].capitalize()  # primeira letra maiúscula
    ...
    def atualizar_conteudo(dados, titulo):
        ...
        if dados:
            for item in dados:
                disciplina = item['disciplina'].capitalize()  # primeira letra maiúscula
                info = item['info']
